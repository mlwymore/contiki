...
//        getInternalSimpleXmlValueBufferContents(parser->vbNextToken));
      ...
      if (delimiter)
      {
        ...
        {
          ns = findNamespaceValue(parser->szAttribute,sizeOfNS);
        }
      }
  ...
  if ( tagDelimiter )
  {
    uri=findNamespaceValue(szTagName,tagDelimiter-szTagName);
    tagDelimiter++;
    parentHandler((SimpleXmlParser) parser, ADD_SUBTAG, uri, tagDelimiter, (char**)attr);
  }
  else
  {
    parentHandler((SimpleXmlParser) parser, ADD_SUBTAG, "", szTagName, (char**)attr);
  }
...
void simpleXmlStartDocument(XmlWriter* xmlWriter, char* buffer, unsigned short size)
{
  ...
  POSITION(xmlWriter) += sprintf(WRITE_BUFFER(xmlWriter),"<?xml version=\"1.0\"?>");
}
