...
import edu.dhbw.andopenglcam.OpenGLCamActivity.RestServiceData;
...
public class OpenGLCamRenderer implements Renderer, PreviewFrameSink{
	...
	//DY
	private static final String TAG = "OpenGLCamRenderer";
	 ...
	 * @param res Resources
	 */
	public OpenGLCamRenderer(Resources res, MarkerInfo markerInfo, OpenGLCamActivity mainActivity)  {
		this.res = res;
	...
	public void onDrawFrame(GL10 gl) {
		...
		if (frameEnqueued) {
			...
			if(!isTextureInitialized) {
				Log.d(TAG, "isTextureInitialized");
				initializeTexture(gl);
			} else {
				//Log.d(TAG, "glTexSubImage2D");
...
//        values[1] = 2.0f;
		...
		Log.i("TAG","Marker:" + mainActivity.currentMarker);
		if ( mainActivity.currentMarker > -1 )
		{
			RestServiceData lastServiceData = mainActivity.lastServiceData[mainActivity.currentMarker];
			if ( lastServiceData != null )
			{
				float volt = 3.0f;
				float cpu, transmit, listen, totalEnergy;
			    cpu=transmit=listen=totalEnergy=0.0f;
		
			    cpu = (1.8f * lastServiceData.cpu + 0.545f * lastServiceData.lpm ) * volt / lastServiceData.cpuAndlpm;
			    transmit = ( (float)lastServiceData.transmit / lastServiceData.cpuAndlpm) * 17.7f * volt;
			    listen = ((float)lastServiceData.listen / lastServiceData.cpuAndlpm) * 20.0f * volt;
			    
			    totalEnergy += cpu;
			    totalEnergy += transmit;
			    totalEnergy += listen;
			    
			    values[0] = (float)lastServiceData.light;
			    values[1] = (float)lastServiceData.temp;
			    values[2] = cpu;
			    values[3] = listen;
			    values[4] = transmit;
			    values[5] = totalEnergy;
			    
			    Log.i("HEBEE","CPU " + cpu +" TRANSMIT " + transmit +" LISTEN " + listen +" TOTAL "+totalEnergy);
			}
		}
	    
		markerInfo.draw(gl, values);
	}
