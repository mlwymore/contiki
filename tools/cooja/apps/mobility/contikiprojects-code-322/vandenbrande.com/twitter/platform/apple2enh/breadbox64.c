...
spacer(unsigned char y)
{
    *(unsigned char *)0x32 = 0x7F;
    cputcxy(0,              y, '[');
    cputcxy(SCREEN_WIDTH-1, y, '[');
    *(unsigned char *)0x32 = 0xFF;
    chlinexy(1, y, SCREEN_WIDTH-2);
}
...
PROCESS_THREAD(twitter_process, ev, data)
{
    ...
    spacer(1);
    cputsxy(20, 5, "Welcome to the Apple II twitter client!");
    ...
    prompt_init(0, SCREEN_HEIGHT-1, credentials, sizeof(credentials), 0, 0, NULL, &p);
    ...
    *credentials_ok = ':';
    ...
    prompt_init(0, SCREEN_HEIGHT-2, buffer, sizeof(buffer), 0, 0, update_status_bar, &p);
